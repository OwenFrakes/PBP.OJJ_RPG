...
func _ready() -> void:
	...
	var eType = typing.new()
	eType.setType("bash")
	enemies[0].setEnemy("Basic", 20, 10, "Club", 10, 10, eType)
	enemies.append(Enemy.new())
	enemies[1].setEnemy("Basic", 30, 5, "Club", 10, 10, eType)
	enemies.append(Enemy.new())
	enemies[2].setEnemy("Basic", 10, 20, "Club", 10, 10, eType)
